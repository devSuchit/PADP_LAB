...
int main ( int argc, char *argv[] ) {
    ...
    /* Initialize MPI.*/
    MPI_Init( &argc, &argv );
    /*Get the number of processes.*/ MPI_Comm_size( MPI_COMM_WORLD, &p ); /*Get the individual process ID.*/ MPI_Comm_rank( MPI_COMM_WORLD, &id ); /*Print a message.*/
    if ( id == 0 )
    {
        printf ( "\n" );
        printf ( "RING_MPI:\n" );
        ...
        ring_io( p, id ); /*Shut down MPI.*/ MPI_Finalize ( ); /*Terminate.*/
        if ( id == 0 )
        {
        printf ( "\n" );
        printf ( "RING_MPI:\n" );
