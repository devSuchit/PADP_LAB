...
//  padp
//
//  Created by Suchit Eswar on 04/12/20.
//
...
int main ( int argc, char *argv[] ) {
    double a;
    double b;
    double error; double exact; int i;
    int master = 0; double my_a; double my_b; int my_id; int my_n; double my_total; int n;
    int p;
    int p_num;
    int source; MPI_Status status; int tag;
    int target;
    double total; double wtime; double x;
    a = 0.0;
    b = 10.0;
    n = 10000000;
    ...
    source = master;
    MPI_Bcast( &my_n, 1, MPI_INT, source, MPI_COMM_WORLD ); /* Process 0 assigns each process a subinterval of [A,B].*/
    if ( my_id == master )
    {
        for ( p = 1; p <= p_num - 1; p++ )
        {
            my_a = ( ( double ) ( p_num - p ) * a
            + ( double ) ( p - 1 ) * b ) / ( double ) ( p_num - 1 );
            target = p;
            tag = 1;
            MPI_Send( &my_a, 1, MPI_DOUBLE, target, tag, MPI_COMM_WORLD );
            my_b = ( ( double ) ( p_num - p - 1 ) * a + ( double ) ( p ) * b )
            / ( double ) ( p_num - 1 ); target = p;
            tag = 2;
        ...
        else
        {
            ...
            for ( i = 1; i <= my_n; i++ )
            {
                ...
                + ( double ) ( i - 1 ) * my_b )
                / ( double ) ( my_n - 1 ); my_total = my_total + f ( x );
            }
        ...
        if ( my_id == master )
        {
            error = fabs ( total - exact ); wtime = MPI_Wtime ( ) - wtime;
            printf ( "\n" );
...
{
    # define TIME_SIZE 40
    static char time_buffer[TIME_SIZE]; const struct tm *tm;
    time_t now;
